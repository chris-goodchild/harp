...
exports.createOutput = (results, options) => {
  ...
  output += createCustomDataTable(results.customData.fileTypes, options.extensions) + '\n'
...
function createCustomDataTable (data, showExtensions) {
  ...
  for (let itemKey in data) {
    ...
    if (!showExtensions || (showExtensions && !item.extensions)) continue
