...
exports.parse = (data, userOptions) => {
    ...
    results.customData = entries.filter(includeCustomData(userOptions.options)).reduce((data, entry) => {
        ...
        for (let type in data.fileTypes) {
            ...
            for (let type in item.complex) {
                let subItem = item.complex[type];
...
function createFileTypes(userOptions) {
    return Object.keys(mimeTypes).reduce((fileTypes, key) => {
        fileTypes[key] = createResult(mimeTypes[key].simple);

        if (userOptions.complex && mimeTypes[key].complex) {
            fileTypes[key].complex = Object.keys(mimeTypes[key].complex).reduce((ext, name) => {
                ext[name] = createResult(mimeTypes[key].complex[name]);
                return ext;
            }, {});
        }
