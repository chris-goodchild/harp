...
exports.parse = (data, userOptions) => {
  ...
  results.pages = data.log.pages.map((page) => {
    return page.title
  })
...
function includeCustomData (options) {
  var pathOptions = options.options || { include: options.include, exclude: options.exclude }
  var includeRegExp = createDomainsRegExp(pathOptions.include)
  var excludeRegExp = createDomainsRegExp(pathOptions.exclude)
